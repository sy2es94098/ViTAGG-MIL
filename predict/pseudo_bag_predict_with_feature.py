...
def test(args, test_df, milnet, d_classifier):
    b_loss = args.b_loss
    m_loss = 1-b_loss
...
if __name__ == '__main__':
    ...
    parser.add_argument('--d_weight', type=str, default=None)
    ...
    d_classifier = abmil.GatedAttention_With_Classifier(args.feats_size).cuda()
    ...
    state_dict_weights = torch.load(args.d_weight)
    ...
    '''    
    state_dict_init = d_classifier.state_dict()
    ...
    d_classifier.load_state_dict(new_state_dict, strict=True)
    ...
    test(args, test_path, milnet, d_classifier)
