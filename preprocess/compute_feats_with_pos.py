...
class BagDataset():
    ...
    def __getitem__(self, idx):
        ...
        img = Image.open(img)
        img_name = temp_path.split(os.sep)[-1]
...
class ToTensor(object):
    def __call__(self, sample):
        ...
        return {'input': img, 'position': sample['position']} 
            ...
            for idx, low_patch in enumerate(low_patches):
                ...
                if len(high_patches) == 0:
                    ...
                    for high_patch in high_patches:
                        patch_name = os.path.basename(high_patch)
                        pos = np.asarray([int(patch_name.split('.')[0].split('_')[0]),int(patch_name.split('.')[0].split('_')[1])])
